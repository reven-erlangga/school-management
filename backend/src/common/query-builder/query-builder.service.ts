...
export class QueryBuilderService {
  ...
  build() {
    ...
    if (this.queryParams.cursor) {
      ...
      prismaQuery.skip = (Number(this.queryParams.page) - 1) * Number(this.queryParams.limit);
    }

    // Build includes (relations)
    const requestedIncludesString = this.queryParams.includes;
    if (requestedIncludesString && typeof requestedIncludesString === 'string') {
    ...
    if (Object.keys(prismaQuery.include).length === 0) delete prismaQuery.include;
    if (Object.keys(prismaQuery.orderBy).length === 0) delete prismaQuery.orderBy;
  ...
  async execute() {
    ...
    if (!this.prisma[this.modelName]) {
      throw new Error(`Model ${this.modelName} does not exist in PrismaService.`);
    }
  ...
  async count() {
    ...
    if (this.useCacheEnabled) {
       ...
       if (cachedResult !== null) { // Count can be 0, so check strict null
         this.logger.debug(`Cache HIT for key: ${cacheKey}`);
         return cachedResult;
       }

       const result = await this.prisma[this.modelName].count({ where: query.where });
       await this.redisService.set(cacheKey, result, this.cacheTTL);
       return result;
    }
