...
async function main() {
  ...
  // 1. Get Institutes
  const oxford = await (prisma as any).institute.findFirst({ where: { name: 'Oxford Polytechnic Institute' } });
  const harvard = await (prisma as any).institute.findFirst({ where: { name: 'Harvard University' } });
  const cambridge = await (prisma as any).institute.findFirst({ where: { name: 'Cambridge International School' } });
  ...
  for (const u of units) {
    const existing = await (prisma as any).unit.findFirst({ where: { name: u.name } });
    if (!existing) {
  ...
  for (const s of streams) {
    const existing = await (prisma as any).stream.findFirst({ where: { name: s.name } });
    if (!existing) {
